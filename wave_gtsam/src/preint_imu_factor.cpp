...
gtsam::Vector PreintegratedImuFactor<PoseVelImuBias>::evaluateError(
  ...
  boost::optional<gtsam::Matrix &> H2) const {
    ...
    if (H1) {
        ...
        // Jacobian wrt imu bias
        H1->block<9, 6>(0, PoseVelImuBias::imu_bias_offset) = D_r_bias_i;
        // adding: [dBiasAcc/dBias_i ; dBiasOmega/dBias_i]
        H1->block<6, 6>(9, PoseVelImuBias::imu_bias_offset) = Hbias_i;
    }
    if (H2) {
        H2->resize(15, 18);
        ...
        // Jacobian wrt imu bias_j is zero
        H2->block<9, 6>(0, PoseVelImuBias::imu_bias_offset).setZero();
        // adding: [dBiasAcc/dBias_i ; dBiasOmega/dBias_i]
        H2->block<6, 6>(9, PoseVelImuBias::imu_bias_offset) = Hbias_j;
    }
