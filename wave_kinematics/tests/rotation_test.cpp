...
public:
  ...
  wave::Rotation rotation1;
  wave::Rotation rotation1_truth;
  wave::Rotation rotation2;
  wave::Rotation rotation3;
  wave::Rotation rotation1_inverse;
  wave::Rotation eye3;

  // A vector v1.
  wave::Vec3 v1;
  // v1 rotated by rotation1.
  wave::Vec3 v1_rotated;
  // True angle axis params for rotation1.
  wave::Vec3 rotation1_angleaxis_params;

  void SetUp() {
    wave::Rotation::fromEulerXYZ(0.1, 0.2, 0.3, rotation1);
    wave::Rotation::fromEulerXYZ(-0.1, -0.2, -0.3, rotation2);
    // rotation3 is the composition of rotation1 and rotation2.
    wave::Rotation::fromEulerXYZ(
      -0.065223037770317, 0.020616131403543, -0.013176146513587, rotation3);
    // rotation1_inverse is the inverse of rotation1.
    wave::Rotation::fromEulerXYZ(-0.037879880513201,
                                 -0.220124031212965,
                                 -0.285771700628461,
                                 rotation1_inverse);
    eye3.rotation_object.setMatrix(1, 0, 0, 0, 1, 0, 0, 0, 1);

    // rotation1_true is set using the underlying element wise initializer.
...
TEST_F(RotationTestFixture, test_from_angle_axis) {
  wave::Rotation rotation_from_angle_axis;
  wave::Rotation::fromAngleAxis(rotation1_angleaxis_params.norm(),
...
TEST_F(RotationTestFixture, test_from_exp_map) {
  wave::Rotation rotation_from_exp_map;
  wave::Rotation::fromExpMap(rotation1_angleaxis_params,
                             rotation_from_exp_map);
...
TEST_F(RotationTestFixture, test_log_map) {
  wave::Vec3 se3_params = rotation1.logMap();
  wave::Vec3 error = rotation1_angleaxis_params - se3_params;
  ASSERT_LE(error.norm(), wave::Rotation::comparision_threshold);
}
...
TEST_F(RotationTestFixture, test_rotate) {
  wave::Vec3 v1_rotated_test = rotation1.rotate(v1);
  wave::Vec3 error = v1_rotated_test - v1_rotated;
  ASSERT_LE(error.norm(), wave::Rotation::comparision_threshold);
}

// Test that inverse rotation of a vector returns correct value.
TEST_F(RotationTestFixture, test_inverse_rotate) {
  wave::Vec3 v1_inverse_rotated_test = rotation1_inverse.inverseRotate(v1);
  wave::Vec3 error = v1_inverse_rotated_test - v1_rotated;
  ASSERT_LE(error.norm(), wave::Rotation::comparision_threshold);
}
...
TEST_F(RotationTestFixture, test_rotate_and_jacobian) {
  ...
  wave::Mat3 Jparam_analytical, Jpoint_analytical;
  rotation1.rotateAndJacobian(v1, Jpoint_analytical, Jparam_analytical);

  // Now get the numerical Jacobians
  wave::Mat3 Jparam_numerical(3, 3), Jpoint_numerical(3, 3);

  // Jacobian wrt the point.
  wave::RotateAndJacobianJpointFunctor Jpoint_functor(rotation1);
  wave::numerical_jacobian(Jpoint_functor, v1, Jpoint_numerical);
  Eigen::MatrixXd error_matrix = Jpoint_analytical - Jpoint_numerical;
  ASSERT_LE(error_matrix.norm(), wave::Rotation::comparision_threshold);

  // Jacobian wrt the rotation params.
  wave::RotateAndJacobianJparamFunctor Jparam_functor(rotation1, v1);
  // Perturbations are about zero vector in the tangent space.
  wave::Vec3 perturbation_vec(0, 0, 0);
  wave::numerical_jacobian(
    Jparam_functor, perturbation_vec, Jparam_numerical);
