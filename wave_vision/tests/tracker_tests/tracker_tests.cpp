...
TEST(TrackerTests, AddImageGetTracks) {
    ...
    cv::Mat image_0 = cv::imread(TEST_IMAGE_0);
    cv::Mat image_1 = cv::imread(TEST_IMAGE_1);
    cv::Mat image_2 = cv::imread(TEST_IMAGE_2);

    // Add image 0. First get time, sleep, and then add later image.
    auto time_early = clock.now();
    sleep(1);
    auto time0 = clock.now();
    tracker.addImage(image_0, time0);
    tracker2.addImage(image_0, time0);
    sleep(1);
    auto time_late = clock.now();

    // Extracting tracks for images that don't exist should throw exceptions.
    ASSERT_THROW(tracker.getTracks(time_early), std::out_of_range);
    ASSERT_THROW(tracker2.getTracks(time_early), std::out_of_range);
    ASSERT_THROW(tracker.getTracks(time_late), std::out_of_range);
    ASSERT_THROW(tracker2.getTracks(time_late), std::out_of_range);

    // The tracks from the first image should be empty (no sequence for tracks)
    std::vector<FeatureTrack> ft0_t1 = tracker.getTracks(time0);
    std::vector<FeatureTrack> ft0_t2 = tracker2.getTracks(time0);
    ...
    // After adding image2 to tracker2, getting tracks for image 0 should throw
    ASSERT_THROW(tracker2.getTracks(time0), std::out_of_range);

    // Get tracks from both images, make sure in both cases they are not empty.
    std::vector<FeatureTrack> ft1_t1 = tracker.getTracks(time1);
    std::vector<FeatureTrack> ft2_t1 = tracker.getTracks(time1);
    std::vector<FeatureTrack> ft1_t2 = tracker2.getTracks(time2);
    std::vector<FeatureTrack> ft2_t2 = tracker2.getTracks(time2);
