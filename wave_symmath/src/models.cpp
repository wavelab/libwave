...
void quadrotor_jacobian(std::vector<GiNaC::ex> &model,
  ...
  GiNaC::symbol ph("ph"), th("th"), ps("ps"), p("p"), q("q"), r("r"), x("x"),
    y("y"), z("z"), vx("vx"), vy("vy"), vz("vz"), Ix("Ix"), Iy("Iy"),
    Iz("Iz"), ktau("ktau"), kt("kt"), tauf("tauf"), taup("taup"),
    tauq("tauq"), taur("taur"), m("m"), g("g"), dt("dt");
  ...
  g4 =
    p + (-((Iz - Iy) / Ix) * q * r - (ktau * p / Ix) + (1 / Ix) * taup) * dt;
  g5 =
    q + (-((Ix - Iz) / Iy) * p * r - (ktau * q / Iy) + (1 / Iy) * tauq) * dt;
  g6 =
    r + (-((Iy - Ix) / Iz) * p * q - (ktau * r / Iz) + (1 / Iz) * taur) * dt;
  g7 = x + vx * dt;
  g8 = y + vy * dt;
  g9 = z + vz * dt;
  g10 = vx +
        ((-kt * vx / m) +
         (1 / m) * (cos(ph) * sin(th) * cos(ps) + sin(ph) * sin(ps)) * tauf) *
          dt;
  g11 = vx +
        ((-kt * vy / m) +
         (1 / m) * (cos(ph) * sin(th) * sin(ps) - sin(ph) * cos(ps)) * tauf) *
          dt;
  g12 = vx + (-(kt * vz / m) + (1 / m) * (cos(ph) * cos(th)) * tauf - g) * dt;

  model = {g1, g2, g3, g4, g5, g6, g7, g8, g9, g10, g11, g12};
  states = {ph, th, ps, p, q, r, x, y, z, vx, vy, vz};
}

void bundle_adjustment_jacobian(std::string file_path) {
  std::ofstream out;
  GiNaC::symbol fx("fx"), fy("fy"), px("px"), py("py"), r11("r11"),
    r12("r12"), r13("r13"), r21("r21"), r22("r22"), r23("r23"), r31("r31"),
    r32("r32"), r33("r33"), x1("x1"), x2("x2"), x3("x3"), c1("c1"), c2("c2"),
    c3("c3");
  GiNaC::ex u, v, w;
