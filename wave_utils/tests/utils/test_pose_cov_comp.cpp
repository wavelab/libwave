...
TEST(PoseCovComp, yprToRotMatrix_test) {
    ...
    ypr << 0.78544, 0.1, 0;
    rot_matrix = yprToRotMatrix(ypr);
    rot_matrix_expected <<  0.7035,   -0.7071,    0.0706,
    ...
    ypr << M_PI * 1.0/3, 1, -2;
    rot_matrix = yprToRotMatrix(ypr);
    rot_matrix_expected <<  0.2702,   -0.0222,   -0.9626,
                            0.4679,   -0.8707,    0.1514,
...
TEST(PoseCovComp, rotMatrixToYPR_test) {
    ...
    rot_matrix <<  0.7035,   -0.7071,    0.0706,
                   0.7036,    0.7071,    0.0706,
                  -0.0998,         0,    0.9950;
    ypr = rotMatrixToYPR(rot_matrix);
    ...
    rot_matrix << 0.2702,   -0.0222,   -0.9626,
                  0.4679,   -0.8707,    0.1514,
                 -0.8415,   -0.4913,   -0.2248;
    ypr = rotMatrixToYPR(rot_matrix);
...
TEST(PoseCovComp, rotMatrixToYPR_degenerate_test) {
    ...
    rot_matrix <<  0.0000,   -0.8415,    0.5403,
                   0.0000,    0.5403,    0.8415,
                  -1.0000,         0,    0.0000;
    ypr = rotMatrixToYPR(rot_matrix);
...
TEST(PoseCovComp, covariance_test_A) {
    ...
    Matrix6x6 m_exp;
    m_exp <<  0.00232,  0.00056, -0.00096,  0.00012,  0.00138, 0.00058,
              0.00056,  0.00272, -0.00132, -0.00152,  0.00112, 0.00064,
             -0.00096, -0.00132,  0.00308,  0.00204, -0.00172, 0.00144,
              0.00012, -0.00152,  0.00204,   0.0032, -0.00116, 0.00184,
              0.00138,  0.00112, -0.00172, -0.00116,  0.00288,  0.0007,
...
TEST(PoseCovComp, covariance_test_B) {
    ...
    // check with the computed results
    Matrix6x6 m_exp;
    m_exp <<  0.00348,  0.00084, -0.00144,  0.00018,  0.00207, 0.00087,
              0.00084,  0.00408, -0.00198, -0.00228,  0.00168, 0.00096,
             -0.00144, -0.00198,  0.00462,  0.00306, -0.00258, 0.00216,
              0.00018, -0.00228,  0.00306,   0.0048, -0.00174, 0.00276,
              0.00207,  0.00168, -0.00258, -0.00174,  0.00432, 0.00105,
...
TEST(PoseCovComp, covariance_test_C) {
    ...
    cov1 <<  0.0854, -0.0113,  0.0153,  0.0191, -0.0074,  0.0271,
    ...
    cov2 <<  0.0216, -0.0126, -0.0011, -0.0107,  0.0020, -0.0154,
    ...
    m_exp <<   0.0939170077, 0.02654455951, 0.04181333697, -0.8900658294,
            0.005086858698, -0.9268740694,
            0.02654455951, 0.09374376512, 0.05351137717,  0.4544374846,
            0.02278949281,  0.4036447693,
            0.04181333697, 0.05351137717,  0.3154435083, -0.7814409313,
            0.117675924, -0.8155600097,
            -0.8900658294,  0.4544374846, -0.7814409313,   36.49441063,
            0.06328226214,   36.44453263,
            0.005086858698, 0.02278949281,   0.117675924, 0.06328226214,
            0.06739192548, 0.05308509534,
            -0.9268740694,  0.4036447693, -0.8155600097,   36.44453263,
            0.05308509534,   36.53097751;
...
TEST(PoseCovComp, covariance_test_D) {
    ...
    cov1 <<  5.1469, -3.9470, -2.8670,  2.7500, -0.5647, -0.4096,
    ...
    cov2 <<  4.9694, -0.9063,  4.7049, -0.1580, -0.9065, -4.8257,
