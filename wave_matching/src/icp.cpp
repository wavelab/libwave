...
ICPMatcher::ICPMatcher(float res, const std::string &config_path) {
    ...
    this->estimate_method = static_cast<ICPMatcher::covar_method> (estimate_method_int);
...
void ICPMatcher::estimate_info() {
    switch (this->estimate_method) {
        case ICPMatcher::covar_method::LUM: this->estimate_lum();
        case ICPMatcher::covar_method::CENSI: this->estimate_censi();
        default: return;
    }
}
...
void ICPMatcher::estimate_censi() {
    if (this->icp.hasConverged()) {
        ...
        sphere_cov.diagonal() << this->lidar_lin_covar, this->lidar_ang_covar, this->lidar_ang_covar,
                                                this->lidar_lin_covar, this->lidar_ang_covar, this->lidar_ang_covar;
...
//                [               0,                   0,                  0,         cos(S6),                   0,        -S4*sin(S6)]
                ...
                d2J_dX2(0, 3) += 2 * Z2 * (sr * sy + cr * cy * sp) +
                                 2 * Z3 * (cr * sy - cy * sr * sp);
                d2J_dX2(1, 3) += -2 * Z2 * (cy * sr - cr * sp * sy) -
                                 2 * Z3 * (cr * cy + sr * sp * sy);
                d2J_dX2(2, 3) += 2 * cp * (Z2 * cr - Z3 * sr);
                d2J_dX2(3, 3) +=
                  (2 * Z2 * (cr * sy - cy * sr * sp) -
                   2 * Z3 * (sr * sy + cr * cy * sp)) *
                    (X1 - Z4 - Z2 * (cr * sy - cy * sr * sp) +
                     Z3 * (sr * sy + cr * cy * sp) + Z1 * cp * cy) -
                  (2 * Z2 * (cr * cy + sr * sp * sy) -
                   2 * Z3 * (cy * sr - cr * sp * sy)) *
                    (X2 - Z5 + Z2 * (cr * cy + sr * sp * sy) -
                     Z3 * (cy * sr - cr * sp * sy) + Z1 * cp * sy) -
                  (2 * Z3 * cr * cp + 2 * Z2 * cp * sr) *
                    (X3 - Z6 - Z1 * sp + Z3 * cr * cp + Z2 * cp * sr) +
                  (Z2 * (sr * sy + cr * cy * sp) +
                   Z3 * (cr * sy - cy * sr * sp)) *
                    (2 * Z2 * (sr * sy + cr * cy * sp) +
                     2 * Z3 * (cr * sy - cy * sr * sp)) +
                  (Z2 * (cy * sr - cr * sp * sy) +
                   Z3 * (cr * cy + sr * sp * sy)) *
                    (2 * Z2 * (cy * sr - cr * sp * sy) +
                     2 * Z3 * (cr * cy + sr * sp * sy)) +
                  (Z2 * cr * cp - Z3 * cp * sr) *
                    (2 * Z2 * cr * cp - 2 * Z3 * cp * sr);

                d2J_dX2(0, 4) +=
                  2 * cy * (Z3 * cr * cp - Z1 * sp + Z2 * cp * sr);
                d2J_dX2(1, 4) +=
                  2 * sy * (Z3 * cr * cp - Z1 * sp + Z2 * cp * sr);
                d2J_dX2(2, 4) +=
                  -2 * Z1 * cp - 2 * Z3 * cr * sp - 2 * Z2 * sr * sp;
                d2J_dX2(3, 4) += -2 * (Z2 * cr - Z3 * sr) *
                                 (X3 * sp - Z6 * sp - X1 * cp * cy +
                                  Z4 * cp * cy - X2 * cp * sy + Z5 * cp * sy);
                d2J_dX2(4, 4) +=
                  (Z1 * cp + Z3 * cr * sp + Z2 * sr * sp) *
                    (2 * Z1 * cp + 2 * Z3 * cr * sp + 2 * Z2 * sr * sp) -
                  (2 * Z3 * cr * cp - 2 * Z1 * sp + 2 * Z2 * cp * sr) *
                    (X3 - Z6 - Z1 * sp + Z3 * cr * cp + Z2 * cp * sr) +
                  2 * cy * cy *
                    pow((Z3 * cr * cp - Z1 * sp + Z2 * cp * sr), 2) +
                  2 * sy * sy *
                    pow((Z3 * cr * cp - Z1 * sp + Z2 * cp * sr), 2) -
                  2 * cy * (Z1 * cp + Z3 * cr * sp + Z2 * sr * sp) *
                    (X1 - Z4 + Z1 * cp * cy - Z2 * cr * sy + Z3 * sr * sy +
                     Z2 * cy * sr * sp + Z3 * cr * cy * sp) -
                  2 * sy * (Z1 * cp + Z3 * cr * sp + Z2 * sr * sp) *
                    (X2 - Z5 + Z2 * cr * cy + Z1 * cp * sy - Z3 * cy * sr +
                     Z3 * cr * sp * sy + Z2 * sr * sp * sy);

                d2J_dX2(0, 5) += 2 * Z3 * (cy * sr - cr * sp * sy) -
                                 2 * Z2 * (cr * cy + sr * sp * sy) -
                                 2 * Z1 * cp * sy;
                d2J_dX2(1, 5) += 2 * Z3 * (sr * sy + cr * cy * sp) -
                                 2 * Z2 * (cr * sy - cy * sr * sp) +
                                 2 * Z1 * cp * cy;
                ...
                d2J_dZdX(0, 4) = 2 * Z6 * cp - 2 * X3 * cp - 2 * X1 * cy * sp +
                                 ...
                                 2 * Z5 * sp * sy;
                d2J_dZdX(1, 4) =
                  -2 * sr * (X3 * sp - Z6 * sp - X1 * cp * cy + Z4 * cp * cy -
                             X2 * cp * sy + Z5 * cp * sy);
                d2J_dZdX(2, 4) =
                  -2 * cr * (X3 * sp - Z6 * sp - X1 * cp * cy + Z4 * cp * cy -
                             X2 * cp * sy + Z5 * cp * sy);
                d2J_dZdX(3, 4) =
                  -2 * cy * (Z3 * cr * cp - Z1 * sp + Z2 * cp * sr);
                d2J_dZdX(4, 4) =
