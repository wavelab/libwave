...
void groundSegmentation::sectorINSAC(int sectorIndex) {
    ...
    // now that the z points are sorted by height, take the NUMSEEDPOINTS worth
    // as the seed
    ...
    while (true) {
        ...
        if (sigPtr[currIdx].range < MAXSEEDRANGE &&
            fabs(sigPtr[currIdx].height) < MAXSEEDHEIGHT)  // close enough to
                                                           // robot and height
                                                           // makese sense in
                                                           // robot locality
        {
    ...
    for (int i = 0; i < (int) currentModel.size(); i++) {
        ...
        for (unsigned int j = 0; j < curCell->binPoints.size(); j++) {
            ...
            if (h < P_TG)  // z heights are close
            {
                ...
                obsSum += Vector3d(curCell->binPoints[j].x,
