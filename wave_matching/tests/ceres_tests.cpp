// Tests for the ceres scan matcher wrapper
...
TEST(ceres_tests, initialization) {
    wave::matching::CeresMatcher matcher(0.1f);
}

// Should return identity (or close to) for no perturbation
TEST(ceres_tests, NullDisplacement) {
    //Setup
    wave::matching::CeresMatcher matcher(0.1f);
...
TEST(ceres_tests, SmallDisplacement) {
    //Setup
    wave::matching::CeresMatcher matcher(0.1f);
    ...
    // Check Result
    //std::cerr << matcher.get_result().matrix() << std::endl << perturb.matrix();
    EXPECT_TRUE(isApprox(result, perturb, 0.1));
}

// Full size pointcloud with 0.3m displacement
TEST(ceres_tests, LargePointCloud) {
    //Setup
    wave::matching::CeresMatcher matcher(0.1f);
    cartographer::sensor::PointCloud point_cloud, dupe;
