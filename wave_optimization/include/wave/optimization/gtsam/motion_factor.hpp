...
class MotionFactor : public gtsam::NoiseModelFactor2<PoseVelBias, PoseVelBias> {
 private:
    const double delt;
 public:
    MotionFactor(gtsam::Key S1, gtsam::Key S2, double delta_t, const gtsam::SharedNoiseModel &model);

    gtsam::Vector evaluateError(const wave::PoseVelBias &m1, const wave::PoseVelBias &m2, boost::optional<gtsam::Matrix &> H1 = boost::none,
