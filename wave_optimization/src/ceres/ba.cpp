...
BundleAdjustment::BundleAdjustment(Mat3 K, MatX x1_pts, MatX x2_pts) {
    this->K = K;
    this->x1_pts = x1_pts;
    this->x2_pts = x2_pts;
...
int BundleAdjustment::solve(MatX pts3d) {
    ...
    for (int i = 0; i < this->x1_pts.rows(); i++) {
        ...
        problem.AddResidualBlock(cost_func,  // cost function
                                 NULL,
                                 this->q[1],   // quaternion
                                 this->c[1],   // camera center
                                 this->x[i]);  // landmark position
    }
