...
std::vector<wave::MatX> generate_data(void) {
  wave::VecX input;
  wave::VecX beta;
  ...
  for (double x = -3.5; x < 3.5; x += 0.1) {
    for (double y = -3.5; y < 3.5; y += 0.1) {
      ...
      outputs.push_back(wave::rosenbrock(input, beta));
    }
  }
...
void test_settings(wave::LMASettings &settings) {
  std::vector<wave::MatX> data;
  wave::MatX x;

  settings.max_iter = 100;
  settings.lambda = 0.01;
  settings.function = LMA_BIND(wave::rosenbrock);
  settings.jacobian = LMA_BIND(wave::rosenbrock_jacobian);
...
TEST(LMAOpt, iterate) {
  wave::LMAOpt opt;
  wave::LMASettings settings;
  wave::VecX beta_before;
  double error;
  std::vector<wave::VecX> data;
