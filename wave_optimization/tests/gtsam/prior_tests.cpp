...
/**
 * now the same but with gps factor
 */
...
TEST(prior, gps_bias) {
    ...
    Eigen::Affine3d measurement(T_local_s1);
    measurement.matrix()(0, 3) = measurement.matrix()(0, 3) + bias_prior(0);
    measurement.matrix()(1, 3) = measurement.matrix()(1, 3) + bias_prior(1);
    measurement.matrix()(2, 3) = measurement.matrix()(2, 3) + bias_prior(2);

    gtsam::Pose3 pose_prior(T_local_s1.matrix());
    gtsam::Pose3 meas_pose(measurement.matrix());
